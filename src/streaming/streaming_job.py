...
def process_batch(batch_df, batch_id):
    ...
    results_df = model.transform(batch_df).select(
        col("Benefit per order").alias("benefit_per_order"), col("Sales per customer").alias("sales_per_customer"),
        col("Order Item Quantity").alias("order_item_quantity"), col("Order Item Product Price").alias("order_item_product_price"),
        col("Order Item Discount").alias("order_item_discount"), col("Order Item Total").alias("order_item_total"),
        col("Order Profit Per Order").alias("order_profit_per_order"), col("distance"), col("Category Name").alias("category_name"),
        col("Order Region").alias("order_region"), col("Shipping Mode").alias("shipping_mode"),
        col("Late_delivery_risk").alias("actual_late_delivery"), col("prediction").alias("predicted_late_delivery"), col("event_time")
    )
    
    results_df.write.jdbc(url=postgres_url, table="streaming_predictions", mode="append", properties=postgres_properties)
    
    aggregated_df = results_df.groupBy(window(col("event_time"), "1 minute"), col("order_region"), col("category_name")).agg(
        count("*").alias("total_orders"), avg("sales_per_customer").alias("avg_sales"), spark_sum("sales_per_customer").alias("total_sales"),
        avg("benefit_per_order").alias("avg_profit"), spark_sum("benefit_per_order").alias("total_profit"),
        spark_sum("predicted_late_delivery").alias("predicted_late_deliveries"), avg("distance").alias("avg_distance")
    )
